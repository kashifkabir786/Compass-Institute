...
if ( isset( $_GET[ 'success' ] ) && $_GET[ 'success' ] == "Added" ) {
    $query_Recordset2 = "SELECT * FROM course ORDER BY course_id DESC LIMIT 1";
    $Recordset2 = mysqli_query( $compass_institute, $query_Recordset2 )or die( mysqli_error( $compass_institute ) );
    $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
    $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );
    $course_id = $row_Recordset2[ 'course_id' ];
    $flag = true;
}
...
if ( ( isset( $_POST[ "MM_insert" ] ) ) && ( $_POST[ "MM_insert" ] == "form1" ) ) {
    ...
    if ( move_uploaded_file( $_FILES[ 'image' ][ 'tmp_name' ], $target ) ) {
        ...
        $popularValue = isset($_POST['popular']) ? 'Yes' : 'No';
        $insertSQL = sprintf( "INSERT INTO `course`(`course_name`, `course_description`, `fees`, `duration`, `popular`, `image`) VALUES (%s, %s, %s, %s, %s, %s)",
            GetSQLValueString( $_POST[ 'course_name' ], "text" ),
            GetSQLValueString( $_POST[ 'course_description' ], "text" ),
            GetSQLValueString( $_POST[ 'fees' ], "text" ),
            GetSQLValueString( $_POST[ 'duration' ], "text" ),
            GetSQLValueString( $popularValue, "text" ),
            GetSQLValueString( $pic, "text" ) );
        $Result = mysqli_query( $compass_institute, $insertSQL )or die( mysqli_error( $compass_institute ) );

        $insertGoTo = "add_course.php?success=Added";
        if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
...
if ( ( isset( $_POST[ "MM_update" ] ) ) && ( $_POST[ "MM_update" ] == "form1" ) ) {
    ...
    $updateSQL = sprintf( "UPDATE `course` SET `course_name` = %s, `course_description` = %s, `fees` = %s, `duration`= %s, `popular` = %s WHERE course_id = %s",
            GetSQLValueString( $_POST[ 'course_name' ], "text" ),
            GetSQLValueString( $_POST[ 'course_description' ], "text" ),
            GetSQLValueString( $_POST[ 'fees' ], "text" ),
            GetSQLValueString( $_POST[ 'duration' ], "text" ),
            GetSQLValueString( $_POST[ 'popular' ], "text" ),
    ...
    $insertGoTo = "add_course.php?success=Updated";
    if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
    }
...
if ( ( isset( $_POST[ "MM_photo" ] ) ) && ( $_POST[ "MM_photo" ] == "form2" ) ) {
  ...
  if ( $imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
    ...
    if ( move_uploaded_file( $_FILES[ 'image' ][ 'tmp_name' ], $target ) ) {
      $insertSQL = sprintf( "UPDATE `course` SET `image` = %s WHERE course_id = %s",
        GetSQLValueString( $pic, "text" ),
        GetSQLValueString( $_POST[ 'course_id' ], "int" ) );
      $Result = mysqli_query( $compass_institute, $insertSQL )or die( mysqli_error( $compass_institute ) );

      $insertGoTo = "add_course.php?success=Image Updated";
      if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
      }
...
<head>
    ...
    <title>Add Course | Compass National Institute</title>
    <!-- plugins:css -->
...
<body>
    <div class="container-scroller">
                ...
                <div class="content-wrapper">
                    <div class="page-header">
                        <h3 class="page-title"> <span class="page-title-icon bg-gradient-primary text-white me-2"> <i
                                    class="mdi mdi-office"></i> </span> Add Course </h3>
                    </div>
                    <div class="row">
                        <div class="col-12 grid-margin">
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-3 mb-1"> <a href="course.php">
                                                <button type="button"
        ...
        if ( isset( $_GET[ 'success' ] ) ) {
            ...
            echo '<div class="alert alert-success">Course ' . $_GET[ 'success' ] . ' Successfully</div>';
            echo '</div>';
        }
        ?>
        ...
        ?>
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-md-4"> <img
                                            src="assets/images/course/<?php echo $row_Recordset2['image'] ?>"
                                        ...
                                        <form action="<?php echo $editFormAction; ?>" method="POST"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" name="course_id"
                                                    value="<?php echo $row_Recordset2['course_id'] ?>" />
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="course_name" class="form-control"
                                                        ...
                                                        value="<?php if($flag) echo $row_Recordset2['course_name'] ?>"
                                                        required>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <textarea rows="4" name="course_description" class="form-control"
                                                        placeholder="Course Description"><?php if($flag) echo $row_Recordset2['course_description'] ?></textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="fees" class="form-control"
                                                        placeholder="Fees"
                                                        value="<?php if($flag) echo $row_Recordset2['fees'] ?>"
                                                        required>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="duration" class="form-control"
                                                        ...
                                                        value="<?php if($flag) echo $row_Recordset2['duration'] ?>"
                                                        required>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <input type="checkbox" name="popular"
                                                    <?php if($flag && $row_Recordset2['popular'] == 'Yes') echo "checked"; ?>
                                                    value="Yes">
                                            </div>
                                        </div>
                                        <?php if(!$flag) { ?>
                ...
                <footer class="footer">
                    <div class="container-fluid d-flex justify-content-between"> <span
                            ...
                            2024 Compass National Institute. All Rights Reserved </span></div>
                </footer>
                <!-- partial -->
            </div>
            <!-- main-panel ends -->
