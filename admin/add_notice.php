...
if ( isset( $_GET[ 'notice_id' ] ) ) {
    ...
    $query_Recordset2 = "SELECT * FROM notice WHERE notice_id = '$notice_id'";
...
if ( isset( $_GET[ 'success' ] ) && $_GET[ 'success' ] == "Added" ) {
    $query_Recordset2 = "SELECT * FROM notice ORDER BY notice_id DESC LIMIT 1";
    $Recordset2 = mysqli_query( $compass_institute, $query_Recordset2 )or die( mysqli_error( $compass_institute ) );
    $row_Recordset2 = mysqli_fetch_assoc( $Recordset2 );
    $totalRows_Recordset2 = mysqli_num_rows( $Recordset2 );
    $notice_id = $row_Recordset2[ 'notice_id' ];
    $flag = true;
}
...
if ( ( isset( $_POST[ "MM_update" ] ) ) && ( $_POST[ "MM_update" ] == "form1" ) ) {
    ...
    $updateSQL = sprintf( "UPDATE `notice` SET `notice` = %s WHERE notice_id = %s",
            GetSQLValueString( $_POST[ 'notice' ], "text" ),
    ...
    $insertGoTo = "add_notice.php?success=Updated";
    if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
        $insertGoTo .= ( strpos( $insertGoTo, '?' ) ) ? "&" : "?";
        $insertGoTo .= $_SERVER[ 'QUERY_STRING' ];
    }
...
<head>
    ...
    <title>Add Notice | Compass National Institute</title>
    <!-- plugins:css -->
...
<body>
    <div class="container-scroller">
                ...
                <div class="content-wrapper">
                    <div class="page-header">
                        <h3 class="page-title"> <span class="page-title-icon bg-gradient-primary text-white me-2"> <i
                                    class="mdi mdi-office"></i> </span> Add Notice </h3>
                    </div>
                    <div class="row">
                        <div class="col-12 grid-margin">
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-3 mb-1"> <a href="notice.php">
                                                <button type="button"
        ...
        if ( isset( $_GET[ 'success' ] ) ) {
            ...
            echo '<div class="alert alert-success">Notice ' . $_GET[ 'success' ] . ' Successfully</div>';
            echo '</div>';
        }
        ?>
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="notice" class="form-control"
                                                        placeholder="Notice"
                                                        value="<?php if($flag) echo $row_Recordset2['notice'] ?>"
                                                        required>
                                                </div>
                                            </div>
                                        </div>
